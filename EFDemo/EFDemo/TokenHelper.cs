...
namespace TokenTest
{
    ...
    {
        private const string Secret = "db05Isj+BXE9NZDy0t8W3TcNekrF+2d/1sFnWG4HnV8TZY30iTOdtVWJG8abWvB1yiOgJuQZdcF2Luqm/Xun==";
        public static string GenerateToken(string userid,int expireMinutes = 480)
        { 
            var symmetricKey = Convert.FromBase64String(Secret);  // 生成二进制字节数组
            var tokenHandler = new JwtSecurityTokenHandler(); // 创建JwtSecurityTokenHandler类用来生成Token
            ...
            { 
                Subject = new ClaimsIdentity(new[] // Token的身份证，类似一个人可以有身份证，户口本
                        {
                            new Claim(ClaimTypes.Name, userid) // 可以创建多个
                        }),
